...
class NewRepositoryScreen extends StatefulWidget {
  ...
  @override
  _NewRepositoryScreenState createState() => _NewRepositoryScreenState();
}

class _NewRepositoryScreenState extends State<NewRepositoryScreen> {
  ...
  Future<void> newFolder(context, String type, {int retry = 0}) async {
    setState(() {
    ...
    if (response.statusCode == 200) {
      ...
      if (!success) {
        print('___HTTP NEW FOLDER SUCCESS FALSE');
      ...
      await Globals.refreshToken()
          .whenComplete(() => newFolder(context, type, retry: 1));
    } else {
      print('___HTTP ERROR NEW FOLDER');
      showDialog(
          context: context,
  ...
  Future<void> newFile(context, String type, File file, {int retry = 0}) async {
    setState(() {
      status = Status.LOADING;
    });
    print(_submitData.toString());
    ...
    if (response.statusCode == 200) {
      ...
      bool success = body['success'];
      if (!success) {
        print('___HTTP NEW FILE SUCCESS = FALSE');
        String message = body['message'];
        showDialog(
            context: context,
            builder: (context) => CustomDialog(title: 'ERRORE', text: message));
      ...
      print('___TRY REFRESH TOKEN');
      await Globals.refreshToken()
          .whenComplete(() => newFile(context, type, file, retry: 1));
    } else {
      print('___HTTP ERROR NEW FILE');
      showDialog(
          context: context,
          builder: (context) => CustomDialog(
              title: 'ERRORE',
    ...
    if (_searchText.isEmpty || _searchText.length < 3)
      ...
      tempList.forEach((searchedTag) {
        if (!_submitData['tags'].contains(searchedTag.id)) {
          print(
              '___SUGGESTED TAG: id:${searchedTag.id} name: ${searchedTag.name}');
      ...
      child: Stack(children: [
        ...
        Scaffold(
          appBar: AppBar(
              ...
              centerTitle: true,
              title: Text(
                type == FileItem.TYPE_FILE ? 'Carica File' : 'Crea Cartella',
                          ...
                          child: Column(
                            children: [
                              ...
                              TextFormField(
                                ...
                                decoration:
                                    InputDecoration(hintText: 'Nome File*'),
                                validator: (value) {
                                  if (value!.isEmpty) {
